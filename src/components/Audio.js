...
class Music extends Component {
  ...
  componentDidUpdate = (prevProps, prevState, snapshot) => {
    ...
    if (
      ...
      this.props.savedLyric !== '' &&
      this.props.savedLyric.includes(this.props.transcript.toLowerCase())
    ) {
  ...
  playPause = () => {
    if (!this.state.toggle) {
      return (
        <button onClick={this.handleClick} id='songButton'>
          {' '}
          Play
        </button>
      );
  ...
  render() {
    return (
      <div id='button-div'>
        {this.playPause()}
        <button onClick={this.handleRedirect} id='redirect'>
          {' '}
          SongSelection{' '}
        </button>
